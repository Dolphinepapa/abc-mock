...
按顺序执行，不可跳步。这是 Explore → Plan → Code → Commit 之上的一层：五步法决定「该不该做、删到什么程度」，四阶段工作流决定「怎么做」。最常见的失败是跳过 1、2 步直接去优化和自动化。

1. **质疑每条需求。**
2. **删。** 能删的零件和流程尽量删；如果事后不需要把删掉的东西加回 10%，说明删得不够狠。
3. **简化和优化。** 只对删剩下的东西做；不要优化一个本来不该存在的东西。
4. **加快周期。** 前三步做完才提速；方向错了，加速等于加速挖坑。
5. **自动化。** 永远放最后；顺序反了就会自动化一堆不该存在的流程。
